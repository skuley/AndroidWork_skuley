...
public class MySQLiteOpenHelper3 extends SQLiteOpenHelper {
    // SQLiteOpenHelper
    // 안드로이드에서 SQLite3 데이터베이스를 좀더 쉽게(편리하게) 사용할 수 있도록 제공되는 클래스
    ...
    // 최초에 앱 설치 이후 데이터베이스가 없는 경우 , 데이터 베이스 생성을 위해 호출되는 콜백
    // 주로 DDl 등 테이블을 생성하는 코드 작성
    @Override
    ...
    // 버젼이 바뀌었을때 기존에 설치 운영되고 있는 데이터베이스를 어떻게 변경할 것인지 작성
