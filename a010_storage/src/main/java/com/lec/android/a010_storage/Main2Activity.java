...
public class Main2Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M &&
            ...
            // onDestroy를 직접 호출 하던가 아니면 finish() 를 해줘도 된다.
        }

        btnSave.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if(!checkExternalStorage()) return; // 외부 메모리 사용 못 하면 종료ㅓ
                ...
                try {
                    ...
                    File f = new File(path, "external.txt"); // 경로와 파일명
    ...
    protected boolean checkExternalStorage(){
        ...
        String msg = "";
        // 외부 메모리 상태
        if(Environment.MEDIA_MOUNTED.equals(state)){ // 외부 메모리가 장착 되었는지 확인 / MEDIA_MOUNTED : 문자열로 "mounted"
            // 장착이 되었다면 읽기, 쓰기 모두 가능
            msg = "외부메모리 읽기 쓰기 모두 가능";
            Log.d("myapp", msg);
            Toast.makeText(this, msg, Toast.LENGTH_SHORT).show();
            return true;
        } else if(Environment.MEDIA_MOUNTED_READ_ONLY.equals(state)){ // "mounted_ro" 읽기만 가능하고 쓰기는 불가능
            // 읽기 전용
            msg = "외부메모리 읽기만 가능";
            Log.d("myapp", msg);
            Toast.makeText(this, msg, Toast.LENGTH_SHORT).show();
            ...
            // 외부 메모리가 없던지 읽기 쓰기 모두 안될 경우
            msg = "외부메모리 읽기 쓰기 모두 불가능 : " + state;
            Log.d("myapp", msg);
            Toast.makeText(this, msg, Toast.LENGTH_SHORT).show();
            return false;
        }
