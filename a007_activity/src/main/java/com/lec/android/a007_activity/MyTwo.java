...
public class MyTwo extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        int a = intent.getIntExtra("num", 0); // "num" 이라는 name으로 넘어온 값
                                                                // 만약 "num" 이라는 name이 없으면 default value로 0을 리턴

        int b = intent.getIntExtra("num2", 0);
        int c = intent.getIntExtra("num3", 999); // "num3"라는 이름이 없었다
        long d = intent.getLongExtra("long", 0);
        String msg = intent.getStringExtra("msg"); // 리턴값이 Object 일경우, 디폴트값 설정 없다 (name이 없으면 null 리턴)

        TextView tv1 = findViewById(R.id.tv1);
        tv1.setText("인텐트 받은값 : " + a + ", " + b + ", " + c + ", " + d + ", " + msg);
