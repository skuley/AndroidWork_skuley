...
public class MainActivity extends AppCompatActivity {
    ...
    @Override
    public boolean onCreateOptionsMenu(Menu menu) { // activity에 menu 객체
        getMenuInflater().inflate(R.menu.menu_main1, menu); // menu 리소스(xml) 로부터 Menu 생성
        Log.d("myapp", "onCreateOptionMenu - 최초 메뉴키를 눌렸을때 호출됨");
        ...
        return true; // false 리턴하면 menu 안보여짐
    }
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        // 메뉴가 보여질때 마다 '로그인'과 '로그아웃' 이 enable/disable 토글되어 보이기
        if(bLog){ // 로그인 상태
            // TODO 다시 보기
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        Log.d("myapp", "onOptionsItemSelected - 메뉴 항목을 클릭했을 때 호출됨");
        showInfo(item);

        // #2. 특정 MenuItem 에 대한 동작
        // 각 색상이 눌렸을때
        // 메뉴를 눌렀을 때 배경화면이 바뀌는것이다
