...
/*  카메라 화면 보여주기 --> SurfaceView 사용
   ...
   그 위에 다른 그림 (아이콤, 마커, 증강현실..) 그리려면 별도의 레이아웃을 위에 포개야 한다
    ...
    API29 부터 android.hardware.Camera 는 deprecated 됨.
        대신 android.ahrdware.Camera2의 객체 사용됨
...
public class Main2Activity extends AppCompatActivity {
    ...
    private class CameraSurfaceView extends SurfaceView implements SurfaceHolder.Callback{
        ...
        // SurfaceView 가 만들어질때 Camera 객체를 참조한 후 미리보기 화면으로
        // Holder 객체 설정
        @Override
        ...
        public void surfaceDestroyed(SurfaceHolder holder) {
            camera.stopPreview();
            // TODO release를 해주는 이유? release가 뭐지?
            camera.release(); // 카메라도 자원이기 때문에 해제
            camera = null;
        }

        // 카메라 orientation 세팅
        // orientation : 현재 보고 있는 카메라가 앞에 있는거 뒤에 있는거 / lanscape 이냐 portrait이냐
