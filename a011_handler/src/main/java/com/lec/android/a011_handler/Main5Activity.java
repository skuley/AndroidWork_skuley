...
public class Main5Activity extends AppCompatActivity {
    ...
    class BackgroundTask extends AsyncTask<Integer, Integer, Integer>{
        ...
        // 백그라운드 작업 -> 반드시 구현
        @Override
        protected Integer doInBackground(Integer... integers) { // 가변 매개변수 리턴타입 배열 --> integers 는 Integer[]
       ...
       // 백그라운드 작업 도중 (여러번) 호출 가능, 진행상황 업데이트, 중간중간 UI 업데이트시 사용
        ...
        protected void onProgressUpdate(Integer... values) { //
            Log.d("myapp", "Progress : " + values[0] + "%"); // publishProgress(i) 가 보낸값
            super.onProgressUpdate(values);

            tvBackValue1.setText("onProgressUpdate : " + values[0]); // ★ 핸들러 없이도 메인 UI 접근 가능 ★
        }

        // doInBackground가 완료되면 호출되는 메소드
        @Override
        protected void onPostExecute(Integer integer) { // doInBackground에서 return한 값이 매개변수로 받는다.
            Log.d("myapp", "Resutl : " + integer);
            super.onPostExecute(integer);

            tvBackValue1.setText("onPosteExecute" + integer); // ★ 핸들러 없이도 메인 UI 접근 가능 ★
        }
    } // AsyncTask
